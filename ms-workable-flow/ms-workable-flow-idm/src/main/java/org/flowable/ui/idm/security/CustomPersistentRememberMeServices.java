...
public class CustomPersistentRememberMeServices extends AbstractRememberMeServices implements CustomRememberMeService {
    ...
    public CustomPersistentRememberMeServices(FlowableIdmAppProperties properties,
        ...
        log.info("Cookie max-age set to {} seconds", tokenMaxAgeInSeconds);
        tokenMaxAgeInMilliseconds = tokenMaxAgeInSeconds * 1000L;

        String domain = cookie.getDomain();
        if (domain != null) {
            log.info("Cookie domain set to {}", domain);
        }
        tokenDomain = domain;
   ...
   /*
    ...
    protected void setCookie(String[] tokens, int maxAge, HttpServletRequest request, HttpServletResponse response) {
        ...
        if (setHttpOnlyMethod != null) {
            ...
            log.debug("Note: Cookie will not be marked as HttpOnly because you are not using Servlet 3.0 (Cookie#setHttpOnly(boolean) was not found).");
        }
